...
typedef struct stDmaWrapper_DmaTransferSetup {
	...
	void*  pFrameBuf;
	//! Fortmat is UBWC =1, Format is Linear =0
	uint16 bIsFmtUbwc;
    //! Should the intermediate buffer be padded.
	//! padded = 1, no padded = 0
	//! This only apply for 8bit format sucha NV12, NV12-4R
...
/*!
 * @brief       Retrieve the DMA status per DMA handle
 *
 * @description Retrieve the DMA status per DMA handle,
 *              this is normally called after the finish frame.
 *
 * @input       hDmaHandle - Engine's DMA Handle
 * @output		pstUbwcCompress - UBWC compression data structure
 *				assign an array uint32 pstUbwcCompress[8] and cast to (void*)pstUbwcCompress;
 * @return      Success: OK
 * @n           Failure: ERR
 */
EXTERN int32 nDmaWrapper_DmaStatus_Get(t_DmaWrapper_DmaEngineHandle hDmaHandle,t_stDmaWrapperDmaStats* pstDmaStatus);
