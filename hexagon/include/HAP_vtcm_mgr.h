...
/**************************************************************************//**
 * @fn HAP_request_async_VTCM
 * @brief Request VTCM memory of desired size and single page requirement
 *        specifying a timeout. Returns pointer to the allocated VTCM memory
 *        when available for the given request in the provided timeout window.
 ...
 *                4KB / 16KB / 64KB / 256KB if single_page_flag == 1.
 *        TIMEOUT:
 *        timeout_us should be a minimum of 200 micro-seconds, otherwise the
 *          api works the same way as HAP_request_VTCM i.e. no timeout window.
 * @param[in] size  size of the request
 * @param[in] single_page_flag  1 for single page requests, 0 otherwise
 * @param[in] timeout_us timeout in micro-seconds, should be greater than 200.
 * @return void* pointer to the created memory block. 0 on failure.
 *****************************************************************************/
